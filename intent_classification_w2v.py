...
X_train_enc     = pad_sequences(sequences, maxlen=10)
seque_test      = tokenizer.texts_to_sequences(X_test) #mode='tfidf')
X_test_enc      = pad_sequences(seque_test, maxlen=10)
...
model_w2v.add(Embedding(vocabulary_size, 300, input_length=10, weights=[embedding_matrix], trainable=True))
model_w2v.add(Dropout(0.4))
model_w2v.add(LSTM(100,recurrent_dropout=0.4))
...
## Fit train data
model_w2v.fit(X_train_enc, y_train_enc,batch_size=batch_size, validation_split=0.1, epochs=20)#, callbacks=callbackss)

### Evaluate the model
score = model_w2v.evaluate(X_test_enc, y_test_enc,
                       batch_size=batch_size, verbose=1)

print('Test accuracy: False embedding 300, drop 0.4, lr 0.01, input_length',10, score[1])
